<template>
  <CrmModal
    ...
    size="medium"
    :title="t('customer.editByCondition')"
    :ok-loading="loading"
  ...
  >
    <n-scrollbar class="max-h-[60vh] pr-[8px]">
      <n-form
        ...
        :label-width="100"
      >
        <div class="mb-[16px] text-[14px] text-[var(--text-n2)]">
          {{ t('customer.editByConditionTotal', { total }) }}
        </div>
        <n-form-item
          path="updateCount"
          :label="t('customer.editCount')"
          :rule="[{ required: true, message: t('customer.editCountInvalid', { max: maxUpdateCount }) }]"
        >
          <div class="w-full">
            <n-input-number
              v-model:value="form.updateCount"
              class="w-full"
              :min="1"
              :max="maxUpdateCount"
              :precision="0"
            ...
            <div class="mt-[4px] text-[12px] text-[var(--text-n4)]">
              {{ t('customer.editCountHint', { limit: MAX_UPDATE_COUNT }) }}
            </div>
          </div>
        </n-form-item>
        <n-form-item
...
<script setup lang="ts">
  ...
  import { FormInst, NForm, NFormItem, NInput, NInputNumber, NScrollbar, NSelect, useMessage } from 'naive-ui';
  import { cloneDeep } from 'lodash-es';

  import { FieldTypeEnum, FormDesignKeyEnum } from '@lib/shared/enums/formDesignEnum';
  import { useI18n } from '@lib/shared/hooks/useI18n';
  import { getNormalFieldValue, getRuleType } from '@lib/shared/method/formCreate';
  import { OpenSeaCustomerTableParams } from '@lib/shared/models/customer';
  ...
  import { batchUpdateOpenSeaCustomerByCondition } from '@/api/modules';
  import { useUserStore } from '@/store';

  import { rules } from '../crm-form-create/config';
  import { SelectMixedOption } from 'naive-ui/es/select/src/interface';

  const MAX_UPDATE_COUNT = 2000;
  ...
  const maxUpdateCount = computed(() => Math.min(props.total, MAX_UPDATE_COUNT));
  ...
  watch(
    ...
    (val) => {
      if (!val) {
        currentForm.value = {};
        form.value.fieldValue = undefined;
        return;
      }
      formRef.value?.restoreValidation();
      const currentFormVal = props.fieldList.find((e) => e.id === val);
      if (!currentFormVal) {
        return;
      }
      ...
      if ([FieldTypeEnum.DATE_TIME, FieldTypeEnum.INPUT_NUMBER].includes(currentFormVal.type)) {
        ...
        defaultValue =
          [FieldTypeEnum.DEPARTMENT, FieldTypeEnum.DATA_SOURCE, FieldTypeEnum.MEMBER].includes(currentFormVal.type) &&
  ...
  function clampUpdateCount() {
    const max = maxUpdateCount.value;
    if (max < 1) {
      form.value.updateCount = 1;
      return;
    }
    form.value.updateCount = Math.min(Math.max(form.value.updateCount || 1, 1), max);
  }
  ...
  watch(showModal, (val) => {
    if (val) {
      form.value = {
        ...initForm,
        updateCount: maxUpdateCount.value > 0 ? maxUpdateCount.value : 1,
      };
      currentForm.value = {};
    }
  });
  ...
  function handleSave() {
    formRef.value?.validate(async (errors) => {
      ...
      const count = form.value.updateCount;
      const max = maxUpdateCount.value;
      if (!count || count < 1 || count > max) {
        Message.warning(t('customer.editCountInvalid', { max }));
        return;
      }
      ...
      try {
        ...
        const data = await batchUpdateOpenSeaCustomerByCondition({
          ...props.queryParams,
          updateCount: count,
        ...
        showModal.value = false;
        Message.success(data.message || '批量编辑任务已提交');
        emit('success');
      } catch (error) {
        console.log(error);
      } finally {
